#!/usr/bin/env python3
"""Deploy firstdata-website to Dokploy using remote ZIP + Dockerfile.dokploy."""
...
EXCLUDE_FILES = {".env", ".env.local", ".deploy-source.zip"}
...
def upload_drop_stub() -> None:
    ...
    with zipfile.ZipFile(stub, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            ".dokploy-remote",
            "Remote-source deploy — full app fetched via SOURCE_ZIP_URL build arg.",
        )
    try:
    ...
    api(
        ...
        "application.saveEnvironment",
        {
            ...
            "env": (
                "NODE_ENV=production\n"
                "PORT=3000\n"
                f"AUTH_SECRET={require_env('AUTH_SECRET', min_len=32)}\n"
...
def main() -> int:
    ...
    print("Uploading source to tmpfiles.org...")
    source_url = upload_zipball(zip_bytes)
    print(f"Source URL: {source_url}")

    print("Configuring Dokploy application...")
    configure(source_url)

    print("Uploading drop stub (Dockerfile.dokploy must already be on server from prior drop/git)...")
    upload_drop_stub()
    ...
    api(
        ...
        {
            "applicationId": APPLICATION_ID,
            "title": "Deploy v1.4.9 remote ZIP",
