...
export default function BlogFeaturedPost({ post }: Props) {
  ...
  return (
    <motion.section
      ...
      initial="hidden"
      animate="show"
      className="border-b border-paper/20 px-4 py-8 sm:px-6 sm:py-10 lg:px-8"
    >
      <div className="mx-auto max-w-6xl">
        <motion.p
          variants={itemReveal}
          dir={dir}
          className={`mb-4 text-[10px] uppercase tracking-widest text-term/70 ${fa ? "font-fa" : "font-mono"}`}
        >
          {ui.featuredLabel}
        </motion.p>
        ...
        <motion.article
          variants={itemReveal}
          className="overflow-hidden border border-term/25 bg-gradient-to-br from-term/[0.06] via-paper/[0.02] to-paper/[0.01]"
        >
          ...
          <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.2fr_1fr] lg:items-center lg:gap-10">
            <div dir={dir}>
              <div className="mb-4 flex flex-wrap items-center gap-2">
                <span className="border border-term/35 bg-term/15 px-2 py-0.5 font-mono text-[10px] text-term">
                  {cat}
                </span>
                <span className={`text-[10px] text-paper/40 ${fa ? "font-fa" : "font-mono"}`}>{date}</span>
              ...
              <h2
                className={`text-2xl leading-tight tracking-tight text-paper sm:text-3xl lg:text-4xl ${fa ? "font-fa" : "font-pixel"}`}
              >
                <Link href={`/blog/${post.slug}`} className="outline-none hover:text-term focus-visible:underline">
                  {content.title}
                </Link>
              </h2>

              <p className={`mt-4 text-base leading-relaxed text-paper/65 sm:text-lg ${fa ? "font-fa" : ""}`}>
                {content.excerpt}
              </p>

              <p className={`mt-4 text-xs text-paper/40 ${fa ? "font-fa" : "font-mono"}`}>
                {ui.writtenBy} {author.name}, {author.role}
              </p>

              <Link
                href={`/blog/${post.slug}`}
                className={`mt-6 inline-flex items-center gap-2 border border-paper/30 px-5 py-2.5 text-sm text-paper/75 transition-colors hover:border-term/50 hover:text-term ${fa ? "font-fa" : "font-mono"}`}
              >
                {ui.readFull}
