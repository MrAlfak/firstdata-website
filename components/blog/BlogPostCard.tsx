...
export default function BlogPostCard({ post, index, featured = false, compact = false }: Props) {
  ...
  return (
    <motion.article
      variants={itemReveal}
      className={`group flex h-full flex-col overflow-hidden border border-paper/20 bg-paper/[0.015] transition-colors duration-200 hover:border-term/30 hover:bg-paper/[0.03] ${
    ...
    >
      ...
      <div className={`flex flex-1 flex-col p-4 sm:p-5 ${compact ? "p-4" : ""}`}>
        <div dir={dir} className="mb-3 flex flex-wrap items-center gap-2">
          <span
            className={`border border-amber/30 bg-amber/10 px-2 py-0.5 font-mono text-[10px] text-amber/90 ${fa ? "font-fa" : ""}`}
          >
            {cat}
          </span>
        ...
        {tags.length > 0 && (
          <div dir={dir} className="mb-3 flex flex-wrap gap-1.5">
            {tags.map((tag) => (
              <span
                key={tag}
                className={`border border-paper/10 px-1.5 py-0.5 font-mono text-[9px] text-paper/35 ${fa ? "font-fa" : ""}`}
              >
                #{tag}
              </span>
            ))}
        ...
        <p
          dir={dir}
          className={`mt-3 flex-1 text-sm leading-relaxed text-paper/55 ${fa ? "font-fa" : ""} ${
        ...
        <Link
          href={`/blog/${post.slug}`}
          dir={dir}
          className={`mt-4 inline-flex items-center gap-2 text-xs uppercase tracking-wider text-paper/50 transition-colors duration-200 group-hover:text-term ${fa ? "font-fa" : "font-mono"}`}
        >
          {ui.readMore}
