...
export type AnimatedTabItem = {
  ...
  badge?: number | string;
  disabled?: boolean;
  /** Panel body. Optional when using list-only mode (`showPanel={false}`). */
  content?: ReactNode;
};
...
interface AnimatedTabsProps {
  ...
  indicatorId?: string;
  /** When false, only the tab strip renders (filters / toggles with external panels). */
  showPanel?: boolean;
}
        ...
        {tabs.map((tab) => {
          ...
          return (
            <TabsTrigger
              ...
              className={cn(
                "relative z-0 h-9 shrink-0 cursor-pointer gap-1.5 rounded-xl border-none bg-transparent px-3.5 text-sm font-medium shadow-none outline-none transition-colors after:hidden data-[state=active]:bg-transparent data-[state=active]:shadow-none",
                isActive
                  ? "text-foreground"
